    ...
    def produce(self):
        ...
        while True:
            ...
            for item in items:
                ...
                item['STT_NM'] = item.pop('stationName') # 대여소 이름
                item['TOT_RACK_CNT'] = item.pop('rackTotCnt') # 거치대 개수
                item['TOT_PRK_CNT'] = item.pop('parkingBikeTotCnt') # 자전거 주차 총 건수
                item['RATIO_PRK_RACK'] = item.pop('shared') # 거치율
                item['STT_LTTD'] = item.pop('stationLatitude') # 위도
                item['STT_LGTD'] = item.pop('stationLongitude') # 경도
